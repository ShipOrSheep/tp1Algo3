...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\If{$m[i][W] == -1$}
			Rproxima = min(Ractual - w[i], r[i])
			$m[i][W] \leftarrow max(DP(i+1, Ractual), DP(i+1, Rproxima))$
		\EndIf
